...
# STEP 2: Download bioRxiv metadata (2015–2025)
metadata_path <- "biorxiv_metadata_2015_2025.rds"

if (!file.exists(metadata_path)) {
  cat("Downloading metadata from bioRxiv...\n")
  data_biorxiv <- mx_api_content(from_date = "2015-01-01", to_date = "2025-04-13", server = "biorxiv")
  saveRDS(data_biorxiv, metadata_path)
} else {
  cat("Loading cached metadata...\n")
  data_biorxiv <- readRDS(metadata_path)
}
...
relevant_categories <- c(
  "Epidemiology", "Immunology", "Microbiology", "Molecular Biology",
  "Genomics", "Pathology", "Pharmacology and Toxicology", "Bioinformatics"
)
...
for (file_name in names(query_list)) {
  ...
  results <- tryCatch(
    mx_search(data = data_biorxiv, query = query, report = TRUE),
    error = function(e) {
  ...
  if (!is.null(results) && nrow(results) > 0) {
    cat("  Results before filtering:", nrow(results), "\n")
    ...
    if (nrow(results_filtered) > 0) {
      results_clean <- results_filtered %>%
        mutate(
          ...
          `Name of Publication/Journal` = category,
          Authors = authors
        ) %>%
        select(
